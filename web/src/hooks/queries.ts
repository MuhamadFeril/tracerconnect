  ...
  export function useUploadAvatar() {
    ...
    return useMutation({
      ...
      onSuccess: (user) => {
        ...
        // Refresh surfaces that mirror the profile (jejaring/alumni lists).
  ...
  /**
   * Reopen an already-submitted kuisioner for updating while the survey is
   * still open (fix mistakes / report a new job). The response becomes
   * in_progress again, then the normal save/submit flow finishes the edit.
   */
  export function useEditSurvey(surveyId: string) {
    const queryClient = useQueryClient()

    return useMutation({
      mutationFn: () => unwrap<SurveyFill>(api.post(`/surveys/${surveyId}/edit`)),
      onSuccess: (data) => {
  ...
  export function useUpdateProfile() {
    ...
    return useMutation({
      ...
      onSuccess: (user) => {
        ...
        setUser(user)
        queryClient.setQueryData(['auth', 'me'], user)
        // Jejaring/alumni surfaces mirror profile data — refresh them so a
        // saved name/photo never stays stale in the directory.
        queryClient.invalidateQueries({ queryKey: ['networking'] })
        queryClient.invalidateQueries({ queryKey: ['alumni'] })
      },
    })
  ...
  export function useHrdDashboard() {
    return useQuery({
      ...
      queryFn: () => unwrap<HrdDashboard>(api.get('/hrd/dashboard')),
    })
  }

  /**
   * New (submitted) application count for the hrd — powers the badge
  ...
  export function useNewApplicationsCount() {
    return useQuery({
      queryKey: ['hrd', 'new-applications-count'],
      queryFn: async () => {
        const data = await unwrap<HrdDashboard>(api.get('/hrd/dashboard'))
  ...
  /**
   * Unified applicant inbox across every vacancy the hrd posted.
   * Filters: search (applicant/vacancy), hiring status, per-vacancy.
   */
  export function useHrdApplications(
    params: { search?: string; status?: string; job_vacancy_id?: string; page?: number } = {},
  ) {
    return useQuery({
      queryKey: ['hrd', 'applications', params],
      queryFn: () => unwrapPage<JobApplication>(api.get('/hrd/applications', { params })),
      placeholderData: keepPreviousData,
    })
  }
