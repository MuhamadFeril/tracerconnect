...
/**
 * Self-service endpoints for the employer portal: employers manage their own
 * vacancies and applicants without going through institution staff/admin.
 */
class EmployerController extends Controller
{
    /**
     * Dashboard summary for the authenticated employer: their own vacancy
    ...
    public function dashboard(Request $request)
    {
        ...
        // choice — keep them out of the employer's review inbox/stats.
        $applicationsQuery = JobApplication::query()
            ->whereHas('vacancy', fn ($q) => $q->where('created_by', $employerId))
        ...
        ], 'Data dashboard employer berhasil diambil');
    }

    /**
     * All applications across the employer's own vacancies — a unified inbox
     * so the employer reviews applicants in one place instead of visiting
    ...
    public function applications(Request $request)
    {
        ...
        $applications = JobApplication::query()
            ...
            // Withdrawn applications left the process by the applicant's own
            // choice — keep them out of the employer's review inbox.
            ->where('status', '!=', 'withdrawn')
            ->with(['vacancy:id,title,company_name,employment_type,location,status', 'alumni.department:id,name', 'alumni.graduationYear:id,year'])
