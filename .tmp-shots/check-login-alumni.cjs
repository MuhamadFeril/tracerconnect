/* CDP check: log in with the newly registered SMK 11 alumni account. */
...
async function main() {
  ...
  const chrome = spawn(
    ...
    [
      ...
      '--user-data-dir=' + path.join(process.env.TEMP || '/tmp', 'tc-alumni-login-profile'),
      'about:blank',
    ],
    { stdio: 'ignore' },
  )
  ...
  await evalJs(
    "(function(){function setVal(sel,val){var el=document.querySelector(sel);if(!el)return false;" +
      ...
      "return setVal('input[name=email]','uji.alumni.smk11@example.test')&&setVal('input[name=password]','Password123!');})()",
  )
  await sleep(300)
  ...
  check('login alumni → /home', url.indexOf('/home') !== -1, url)
  const body = await evalJs('document.body.innerText')
  check('portal alumni (Beranda SMK 11) tampil', (body || '').indexOf('Beranda') !== -1)
  check('nama alumni tampil di halaman', (body || '').indexOf('Uji Alumni SMK11') !== -1)
  await shot('09-alumni-beranda.png')
